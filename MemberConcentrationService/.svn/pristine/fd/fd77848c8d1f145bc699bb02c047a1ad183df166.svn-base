...
public class CommonServiceImpl implements CommonService {
	...
	public boolean selectCinemaNo(String placeNo) throws Exception {
		StringBuffer sql = new StringBuffer("SELECT count(*) FROM T1005_CINEMA T1005 WHERE T1005.PLACENO = ? ");
	...
	@Override
	public C0003_MEM_MEMBER selectMemberAdd(String memberno, String idnum, String placeno)
			throws Exception {
		StringBuffer sql = new StringBuffer("select * from C0003_MEM_MEMBER t where t.placeno=? and t.memberno=?");
		if(StringUtils.isNotBlank(idnum)){
			sql.append("or t.idnum='"+idnum+"' ");
		}
		return dao.findByWhere(sql.toString(), new Object[]{placeno,memberno}, C0003_MEM_MEMBER.class);
	}
	@Override
	public C0003_MEM_MEMBER selectMemberUpdate(String memberno, String idnum,
			String placeno) throws Exception {
		if(StringUtils.isNotBlank(idnum)){
			StringBuffer sql = new StringBuffer("select * from C0003_MEM_MEMBER t where t.placeno=? and t.memberno!=?");
				sql.append("and t.idnum='"+idnum+"' ");
				return dao.findByWhere(sql.toString(), new Object[]{placeno,memberno}, C0003_MEM_MEMBER.class);
			}
		return null;
	}
	...
	public C0001_MEM_ACCOUNT selectMemberAccountCardno(String cardno,
			String placeno) throws Exception {
		StringBuffer sql = new StringBuffer("select * from C0001_MEM_ACCOUNT t where t.placeno=? and t.cardno=?");
		return dao.findByWhere(sql.toString(), new Object[]{placeno,cardno}, C0001_MEM_ACCOUNT.class);
	}
	@Override
	public C0001_MEM_ACCOUNT selectMemberAccountMemoryid(String memoryid,
			String placeno) throws Exception {
		StringBuffer sql = new StringBuffer("select * from C0001_MEM_ACCOUNT t where t.placeno=? and t.memoryid=?");
		return dao.findByWhere(sql.toString(), new Object[]{placeno,memoryid}, C0001_MEM_ACCOUNT.class);
	}
}
