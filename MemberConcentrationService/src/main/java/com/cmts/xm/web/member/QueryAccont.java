...
public class QueryAccont {
	...
	public @ResponseBody String queryAccont(String postdata, HttpServletRequest request, HttpServletResponse response){
		...
		try {
			...
			log.info("queryAccont传入参数：accountno=" + jsonBean.getAccountno() + ",cardno=" + jsonBean.getCardno() + ",memoryid=" + jsonBean.getMemoryid()+ ",placeno=" + jsonBean.getPlaceno()+ ",verifyInfo=" + jsonBean.getVerifyInfo());
			if((StringUtils.isBlank(jsonBean.getAccountno())&&StringUtils.isBlank(jsonBean.getCardno())&&StringUtils.isBlank(jsonBean.getMemoryid()))||
			...
			C0001_MEM_ACCOUNT account= queryAccontService.getAccountData(jsonBean.getAccountno(),jsonBean.getCardno(),jsonBean.getMemoryid(),jsonBean.getPlaceno());
			if(account == null){
